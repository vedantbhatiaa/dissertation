...
def render_tip_progress_report(sector_df, master_df=None):
    ...
    # NOTE: the page-level title used to be rendered here via st.title(...).
    # It's now shown by the navy hero card in pages/page_sector_reports.py
    # (matches the My Dashboard / Benchmarking / Analysis header style),
    # so it isn't repeated here to avoid a duplicate heading.
    tab3, tab4, tab5 = st.tabs([
        "Pathway 3 — Manufacturing",
        "Pathway 4 — Employees",
        "Pathway 5 — TRWP",
    ])
...
def plot_water_withdrawals_chart(data):
    ...
    fig.update_layout(
        ...
        barmode="stack",
        showlegend=False,
        xaxis=dict(domain=[0.32, 1.0], tickmode="array", tickvals=x, ticktext=year_labels, showline=True, linewidth=1, linecolor=AXIS_COLOR, showgrid=False, ticks="", tickfont=dict(size=11, color=MUTED_TEXT), zeroline=False),
    ...
    fig.add_annotation(x=0.01, y=0.25, xref="paper", yref="paper", text="■ Stress withdrawals", showarrow=False, font=dict(size=11, color="#b9c8d4"), align="left", xanchor="left")
    fig.add_annotation(x=0.01, y=0.15, xref="paper", yref="paper", text="■ Non-stress withdrawals", showarrow=False, font=dict(size=11, color="#9cae7b"), align="left", xanchor="left")
    fig.add_annotation(x=0.01, y=0.05, xref="paper", yref="paper", text="—○— Water intensity", showarrow=False, font=dict(size=11, color=LINE_COLOR), align="left", xanchor="left")
...
def plot_electricity_chart(data):
    ...
    fig.update_layout(
        ...
        paper_bgcolor=REPORT_BG,
        showlegend=False,
        xaxis=dict(domain=[0.40, 1.0], tickmode="array", tickvals=x, ticktext=year_labels, showline=True, linecolor=AXIS_COLOR, showgrid=False, ticks="", tickfont=dict(size=11, color=MUTED_TEXT), zeroline=False),
    ...
    fig.add_annotation(x=0.01, y=0.18, xref="paper", yref="paper", text="■ Non-renewable electricity (GJ)", showarrow=False, font=dict(size=11, color=TIP_SAND), align="left", xanchor="left")
    fig.add_annotation(x=0.01, y=0.08, xref="paper", yref="paper", text="■ Renewable electricity (GJ)", showarrow=False, font=dict(size=11, color=TIP_SPRUCE), align="left", xanchor="left")
...
def plot_simple_bar_report_chart(years, values, title, y_label, bottom_values=False):
    ...
    if bottom_values:
        # Reserve more vertical room below the plot area (was 0.18) so the
        # value labels no longer sit on top of the year tick labels.
        yaxis_layout["domain"] = [0.26, 1.0]
    fig.update_layout(title=dict(text=title, x=0.02, font=dict(size=13)), height=CHART_HEIGHT, margin=dict(l=78, r=50, t=38, b=58) if bottom_values else CHART_MARGIN_SIMPLE, plot_bgcolor=REPORT_BG, paper_bgcolor=REPORT_BG, showlegend=False, xaxis=dict(tickmode="array", tickvals=x, ticktext=[str(y) for y in years], showline=True, linecolor=AXIS_COLOR), yaxis=yaxis_layout)
    if bottom_values:
        for i, x_pos in enumerate(x):
            # Moved down from y=0.07 (was overlapping the year tick labels
            # just below the axis) to sit in its own clear band.
            fig.add_annotation(x=x_pos, y=0.0, xref="x", yref="paper", text=_format_value(values[i]), showarrow=False, font=dict(size=11, color=MUTED_TEXT))
    else:
        for i, x_pos in enumerate(x):
