...
# Canonical write helpers live in utils/data_utils.py. The save path uses the
# aliased _save_submission_to_csv_fixed; the startup fold-in uses the imported
# _migrate_supplementary_to_master. app.py no longer keeps local copies of these.
from utils.data_utils import (
    _save_submission_to_csv as _save_submission_to_csv_fixed,
...
# ═══════════════════════════════════════════════════════════════════════════════
# SUPPLEMENTARY DATA — RETIRED
# The fields outside TemplateInputs (H&S, Diversity, SBT, Water detail, Coal
# breakdown) used to live in a separate ESG_SUPPLEMENTARY.csv. They now live as
# columns IN the master wide CSV. Reads go through utils.data_utils._load_supple-
# mentary (master-backed); writes happen via _save_submission_to_csv(supp=...).
# _migrate_supplementary_to_master (imported from utils.data_utils) is a one-time,
# idempotent fold-in for any legacy ESG_SUPPLEMENTARY.csv still on disk.
# ═══════════════════════════════════════════════════════════════════════════════
...
# NOTE: CLIENTS is already set above (line ~487) from cfg.load_clients() —
# a second hardcoded copy used to live here and silently overwrite it with
# stale placeholder company names that didn't exist anywhere in the master
# data. That's the actual reason logging in as any demo client always showed
# "No data yet" regardless of what was in the master CSV. Removed; config.py
# is now the single source of truth for the client login list.
...
def _get_easy_auth_email() -> str | None:
    """
    Read the signed-in user's email from the header App Service Easy Auth
    forwards once Entra ID login succeeds (X-MS-CLIENT-PRINCIPAL-NAME).
    Requires Streamlit >= 1.37 for st.context.headers. Returns None locally
    (no Azure front door in front of `streamlit run`), which is exactly what
    lets show_login()'s manual form keep working for local development.
    """
    try:
...
def try_easy_auth_login() -> bool:
    """
    Called once per script run, before the login gate. If Easy Auth has
    already authenticated this browser session with Entra ID, populate
    session_state exactly the way the manual login form used to, and
    return True so the router skips straight past show_login().
    """
    if st.session_state.get("authenticated"):
        return True
    email_l = _get_easy_auth_email()
    if not email_l:
        return False
    is_dss    = DSS_DOMAIN in email_l
    is_client = email_l in CLIENTS
    if not is_dss and not is_client:
        # Signed in with Entra ID but not on the client/DSS allowlist —
        # do NOT silently authenticate; show an explicit access-denied
        # message instead of falling through to the dev-mode form (which
        # would be confusing once Easy Auth is the front door in Azure).
        st.error(
            f"Signed in as **{email_l}**, but this account isn't provisioned "
            "on the TIP platform. Contact your dss+ administrator to be added."
        )
        st.stop()
    name_parts = email_l.split("@")[0].replace(".", " ").split()
...
def show_login():
    ...
    # This form only renders when try_easy_auth_login() found no Entra ID
    # session — i.e. local development (`streamlit run app.py` on a laptop
    # with no App Service Easy Auth in front). In Azure, users never see
    # this; they're redirected straight to the Microsoft login page instead.
    st.caption("🔧 Local dev sign-in — production uses Microsoft login (Easy Auth)")
...
def show_sidebar():
    with st.sidebar:
        ...
        # ── DSS+ INTERNAL navigation ──────────────────────────────────────────
            ...
            _nav_item("benchmarking",   "Benchmarks")
            # PAUSED — AI Assistant and Document Library are being reworked on a
            # separate branch. Not deleted, just hidden from navigation for now.
            # Restore by uncommenting these two lines (and the matching import +
            # router branches below):
...
# PAUSED — see nav-item comment above. Re-enable together with the sidebar
# entries and the router branches below when this work resumes.
...
if not st.session_state.authenticated:
    ...
    # NOTE: this used to be gated ONLY by a session-state flag, which resets
    # on every fresh login/restart — meaning it would re-run (and re-do
    # whatever it does) every single time someone logged in, not actually
    # "once". Added a persistent on-disk marker so it truly only runs once.
    _migration_marker = Path("data_storage/master/.supp_migration_done")
    if not st.session_state.get("_supp_migrated") and not _migration_marker.exists():
        try:
    ...
    # PAUSED — see nav-item / import comments above.
