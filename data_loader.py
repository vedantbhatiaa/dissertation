...
WIDE_COL_TO_FIELD = {
    ...
    "Total amount of waste":                           "waste_total",
    "Amount of waste sent to recovery":                "waste_recovery",
    # ── Canonical master wide CSV column names — "Energy - <fuel>" schema ────
    # The master CSV uses these exact column names. Mapped here so
    # get_step_data reconstructs TemplateInputs correctly from the wide CSV.
    # IMPORTANT:
    #   "Energy - Total Coal" is intentionally NOT mapped — the 3 granular
    #   sub-type fields below are the authoritative source. Mapping the total
    #   would write to coal_sub, which formula_engine ignores whenever any
    #   granular field is non-zero (correct), but it would also shadow the
    #   granular values if it appears later in column order (incorrect).
    #   "Energy - Fuel Oil" (blended A+C total) is intentionally NOT mapped —
    #   mapping it to fuel_oil_heavy_a double-counts when Heavy C is also
    #   present as a separate column (Energy - Fuel Oil = Heavy A + Heavy C).
    "Total amount of Waste":                           "waste_total",
    "Energy - Natural Gas":                            "nat_gas",
    "Energy - Sub bituminous coal":                    "coal_sub_bituminous",
    "Energy - Brown coal briquettes":                  "coal_brown_briquettes",
    "Energy - Other bituminous coal":                  "coal_other_bituminous",
    "Energy - Propane":                                "propane",
    "Energy - Fuel Oil Heavy A":                       "fuel_oil_heavy_a",
    "Energy - Fuel Oil Heavy C":                       "fuel_oil_heavy_c",
    "Energy - Diesel":                                 "diesel",
    "Energy - Petrol":                                 "petrol",
    "Energy - Biomass":                                "biomass",
    "Energy - Waste tires":                            "waste_tires_mt",
    "Energy - LPG":                                    "lpg",
    "Energy - Other":                                  "other_fuels",
    # ── legacy wide names (still in some Excel sheets) ────────────────────────
...
TEMPLATE_INPUT_FIELDS = {
    ...
    "nat_gas","coal_sub","propane","fuel_oil_heavy_a","fuel_oil_heavy_c",
...
def _clean_df(df: pd.DataFrame) -> pd.DataFrame:
    """Shared cleaning: parse Year, coerce numerics, add alias columns."""
    ...
    if _is_wide_format(df):
        ...
        # ── Alias + derived columns ──────────────────────────────────────────
        # Batch all new columns into one pd.concat to avoid the PerformanceWarning
        # caused by adding columns one-by-one to a highly-fragmented 700+ col DF.
        # Pages can read either the canonical name or the alias — both resolve.
        _new: dict = {}

        # Waste aliases — add BOTH directions so pages work regardless of
        # which column name is present in the master CSV.
        # Master may store either the canonical or legacy name depending on
        # which save path created it.
        if "Total amount of Waste" in df.columns and "Total Waste" not in df.columns:
            _new["Total Waste"] = df["Total amount of Waste"]
        elif "Total Waste" in df.columns and "Total amount of Waste" not in df.columns:
            _new["Total amount of Waste"] = df["Total Waste"]

        if "Amount of waste sent to recovery" in df.columns and "Waste Recovered" not in df.columns:
            _new["Waste Recovered"] = df["Amount of waste sent to recovery"]
        elif "Waste Recovered" in df.columns and "Amount of waste sent to recovery" not in df.columns:
            _new["Amount of waste sent to recovery"] = df["Waste Recovered"]

        # Waste recovery rate — try both source column names, normalise 0-1 → 0-100
        if "Waste recovery %" in df.columns:
            _new["Waste_Recovery_Rate_%"] = df["Waste recovery %"] * 100
        elif "Recovery Rate" in df.columns and "Waste_Recovery_Rate_%" not in df.columns:
            # "Recovery Rate" may already be 0-100 or 0-1 — detect and normalise
            _rr_max = df["Recovery Rate"].max()
            if pd.notna(_rr_max) and _rr_max <= 1.01:
                _new["Waste_Recovery_Rate_%"] = df["Recovery Rate"] * 100
                _new["Waste recovery %"]      = df["Recovery Rate"]
            else:
                _new["Waste_Recovery_Rate_%"] = df["Recovery Rate"]
                _new["Waste recovery %"]      = df["Recovery Rate"] / 100

        # ISO cert % 0-1 → 0-100
        if "% certified sites" in df.columns:
            _new["ISO_Certification_%"] = df["% certified sites"] * 100

        # Renewable electricity share — derived
        if ("Renewable Electricity Purchased" in df.columns
                and "Non-Renewable Electricity Purchased" in df.columns):
            _total_elec = (df["Renewable Electricity Purchased"]
                           + df["Non-Renewable Electricity Purchased"])
            _new["Renewable_Electricity_Share_%"] = (
                df["Renewable Electricity Purchased"]
                .div(_total_elec.replace(0, float("nan")))
                .mul(100).round(2)
            )

        # Waste intensity kg/t — benchmarking reads this column
        if "Total amount of Waste" in df.columns and "Production" in df.columns:
            _new["Waste_Intensity_kg_t"] = (
                df["Total amount of Waste"]
                .div(df["Production"].replace(0, float("nan")))
                .mul(1000).round(4)
            )

        # H&S + diversity % stored as 0-1 → multiply to 0-100 in-place.
        # Guard: if already 0-100 (e.g. _clean_df called twice), skip.
        for _raw in ["HS External Audit %", "HS Internal Audit %",
                     "Female Employees %", "Female Board %"]:
            if _raw in df.columns:
                _col_max = df[_raw].max()
                if _col_max is not None and not __import__("math").isnan(float(_col_max or 0)):
                    if float(_col_max) <= 1.5:  # still 0-1 fraction → multiply
                        df[_raw] = df[_raw] * 100
                    # else: already 0-100, skip (idempotent)

        # Concat all new cols at once — avoids DataFrame fragmentation warning
        if _new:
            _add = pd.DataFrame(_new, index=df.index)
            # Only add columns that don't already exist
            _add = _add[[c for c in _add.columns if c not in df.columns]]
            if not _add.empty:
                df = pd.concat([df, _add], axis=1)
        # ── End alias columns ─────────────────────────────────────────────────
    else:
...
def load_consolidated() -> pd.DataFrame:
    ...
    Load order:
      1. data_storage/master/ESG_MASTER_WIDE_ALL_COMPANIES_*.csv — primary
         source, kept up-to-date by the app's save path.
      2. data_storage/raw/ESG_MASTER_WIDE_ALL_COMPANIES_*.csv
