...
def page_analysis():
    ...
    # ── Hero card (matches My Dashboard / Benchmarking style) ─────────────────
    NAVY_DARK = "#0F2540"
    NAVY_MID  = "#1A3A5C"
    st.markdown(f"""
    <style>
    ...
    .analysis-hero-eyebrow {{ font-size: 11px; letter-spacing: .06em; text-transform: uppercase;
                             color: rgba(255,255,255,.55); margin-bottom: 6px; }}
    .analysis-hero-title {{ font-size: 30px; font-weight: 800; color: white; line-height: 1.2; }}
    .analysis-hero-sub {{ font-size: 12.5px; color: rgba(255,255,255,.7); margin-top: 4px; }}
    .analysis-hero-year-label {{ font-size: 10px; color: rgba(255,255,255,.5); text-align: right;
                                text-transform: uppercase; letter-spacing: .05em; }}
    .analysis-hero-year {{ font-size: 30px; font-weight: 700; color: white; text-align: right; line-height: 1; }}
    </style>
    """, unsafe_allow_html=True)

    _hero_title = _html.escape(overlay_company) if overlay_company else "All Companies"
    st.markdown(f'''<div class="analysis-hero">
        <div>
            <div class="analysis-hero-eyebrow">Tire Industry Platform</div>
            <div class="analysis-hero-title">{_hero_title}</div>
            <div class="analysis-hero-sub">ESG Analysis &amp; Trends</div>
        </div>
        <div>
            <div class="analysis-hero-year-label">Reporting Year</div>
            <div class="analysis-hero-year">{overlay_year}</div>
        </div>
    ...
    def _sector_renew_pct():
        """Sector-wide renewable electricity share, derived directly from the
        raw purchased-electricity columns (confirmed correct — same columns
        page_my_dashboard.py already reads successfully) rather than trusting
        a precomputed "Renewable_Electricity_Share_%" column whose existence
        in the real wide CSV schema we can't confirm without seeing it."""
        r = _sector("Renewable Electricity Purchased")
        n = _sector("Non-Renewable Electricity Purchased")
        if r is None or n is None:
            return None
        tot = (r.fillna(0) + n.fillna(0)).replace(0, np.nan)
        return r / tot * 100

    def _sector_waste_recov_pct():
        """Sector-wide waste recovery rate, derived from the two canonical
        waste columns (same names page_my_dashboard.py's "# canonical master
        col" comments confirm) instead of a "Waste_Recovery_Rate_%" column."""
        rec = _sector("Amount of waste sent to recovery")
        tot = _sector("Total amount of Waste")
        if rec is None or tot is None:
            return None
        return rec / tot.replace(0, np.nan) * 100

    def _sector_iso_pct():
        """Sector-wide ISO 14001 certification rate, derived from raw site
        counts instead of a possibly-nonexistent "ISO_Certification_%" column."""
        iso = _sector("ISO 14001 sites")
        tot = _sector("Total no. of sites")
        if iso is None or tot is None:
            return None
        return iso / tot.replace(0, np.nan) * 100

    def _co_waste_recov_pct(co):
        """Per-company waste recovery rate, derived from the same two
        canonical waste columns as the sector-wide version above."""
        rec = _co_series(co, "Amount of waste sent to recovery")
        tot = _co_series(co, "Total amount of Waste")
        if rec is None or tot is None:
            return None
        return rec / tot.replace(0, np.nan) * 100
    ...
    renew_pct     = _safe(_sector_renew_pct(),                     state.LONG_DATA["renew_pct"])
    waste_recov   = _safe(_sector_waste_recov_pct(),                state.LONG_DATA["waste_recov"])
    iso_cert      = _safe(_sector_iso_pct(),                       [93.0]*len(yrs_int))
    # NOTE: were "Total Waste" / "Waste Recovered" — neither matches the
    # canonical master-CSV column names confirmed elsewhere in this codebase
    # (page_my_dashboard.py explicitly flags these exact strings as the
    # "canonical master col"), so this was silently always falling through
    # to demo data. Fixed to the real column names.
    waste_total_v = _safe(_sector("Total amount of Waste"),        [v*330000 for v in state.LONG_DATA["prod"]])
    waste_recov_a = _safe(_sector("Amount of waste sent to recovery"), [v*280000 for v in state.LONG_DATA["prod"]])
    ...
    def _stackabs(xs, traces, title="", h=430):
    ...
    iso_cert     = _safe(_sector_iso_pct(),                        [97.0]*len(yrs_int))

    # Energy mix components (absolute GJ, then computed as %)
    # NOTE: confirmed via the long-form master (KPI_Name column) that energy
    # and CO2 both have parallel rows for these fuels ("Energy - Natural Gas"
    # vs "CO2 - Natural Gas", etc.) — these must use the "Energy - " prefix
    # specifically, not the bare fuel name (which doesn't exist as a column
    # at all) and not the CO2-sector reading.
    ...
    _natg        = _safe(_sector("Energy - Natural Gas"),           [2.3e8]*len(yrs_int))
    _coal        = _safe(_sector("Energy - Total Coal"),            [5e5]*len(yrs_int))
    _diesel      = _safe(_sector("Energy - Diesel"),                [2e6]*len(yrs_int))
    _lpg         = _safe(_sector("Energy - LPG"),                   [1.5e7]*len(yrs_int))
    _bio         = _safe(_sector("Energy - Biomass"),                [1e5]*len(yrs_int))
    ...
    with tab_energy:
        ...
        with c2:
            ...
            st.plotly_chart(_dual(
                yrs, plot_pidx, "Production level (%)", TC["bar_blue"],
    ...
    with tab_p4:
        ...
        fig_wabs.update_layout(
            ...
            margin=dict(l=130, r=50, t=50, b=30),
            showlegend=False,   # row annotations below serve as the legend
        ...
        fig_wabs.add_annotation(x=0.01, y=0.28, xref="paper", yref="paper",
            text="■ Sent for recovery (t)", showarrow=False,
            font=dict(size=10, color="#7BAF74", family="Arial"), xanchor="left")
        fig_wabs.add_annotation(x=0.01, y=0.13, xref="paper", yref="paper",
            text="■ Sent for disposal (t)", showarrow=False,
            font=dict(size=10, color="#2D4A5A", family="Arial"), xanchor="left")
        for _xi, _r, _d in zip(_wt_xi, waste_recov_a, _dis_abs):
            fig_wabs.add_annotation(x=_xi, y=0.28, xref="x", yref="paper",
                text=f"{int(_r):,}" if _r else "", showarrow=False,
                font=dict(size=9, color="#374151", family="Arial"))
            fig_wabs.add_annotation(x=_xi, y=0.13, xref="x", yref="paper",
                text=f"{int(_d):,}" if _d else "", showarrow=False,
                font=dict(size=9, color="#374151", family="Arial"))
        ...
        with c4:
            ...
            st.plotly_chart(_dline(
                ...
                right_y=True, s1f=".0f", s2f=".0f", h=430,
    ...
    with tab_people:
        ...
        def _pg_live_pct_and_counts(num_col, den_col):
            """Sector-aggregate %, derived from raw headcount/site columns —
            NOT a precomputed "%" column. Dummy data has shown those can
            contain corrupted values (800%+) from bad entries; summing the
            actual raw counts and dividing is both more robust and lets the
            chart show real headcounts alongside the percentage. Clamped to
            [0,100] as a hard safety net regardless of root cause upstream."""
            pct_list, num_list, den_list = [], [], []
            for y in yrs_int:
                sub = state.CONSOLIDATED_DF[state.CONSOLIDATED_DF["Year"] == y] if not state.CONSOLIDATED_DF.empty else None
                if overlay_company and overlay_company != "All Companies" and sub is not None:
                    sub = sub[sub["Company"] == overlay_company]
                if sub is None or sub.empty or num_col not in sub.columns or den_col not in sub.columns:
                    pct_list.append(None); num_list.append(None); den_list.append(None)
                    continue
                num_sum = pd.to_numeric(sub[num_col], errors="coerce").sum()
                den_sum = pd.to_numeric(sub[den_col], errors="coerce").sum()
                if den_sum and den_sum > 0:
                    pct = round(min(max(num_sum / den_sum * 100, 0), 100), 1)
                else:
                    pct = None
                pct_list.append(pct)
                num_list.append(int(num_sum) if pd.notna(num_sum) else None)
                den_list.append(int(den_sum) if pd.notna(den_sum) else None)
            return pct_list, num_list, den_list

        def _pg_bar_chart(xs, s1_pct, s1_num, s1_den, s1_label, s1_color,
                          s2_pct, s2_num, s2_den, s2_label, s2_color, title, yt):
            """Grouped year-on-year bar pair — % shown inside each bar, with
            the actual headcount ratio (e.g. "23/440") printed in a row
            below the x-axis. This now mirrors _dual() COMPLETELY, including
            the numeric x_idx + linear axis type — the previous version kept
            _tlayout's category-type axis while also restricting its domain,
            and that combination is what was actually breaking the render
            (only the first year visible, garbled overlapping annotations).
            Category + domain-restriction don't mix in this chart library;
            numeric index + linear does, because that's what _dual() uses."""
            n     = len(xs)
            x_idx = list(range(n))

            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=x_idx, y=s1_pct, name=s1_label, marker_color=s1_color, marker_line_width=0,
                width=0.35, offset=-0.2, cliponaxis=False,
                text=[f"{v:.0f}%" if v is not None else "" for v in s1_pct],
                textposition="inside", textfont=dict(size=10, color="white", family="Arial"),
                hovertemplate=f"{s1_label}: %{{y:.1f}}%<extra></extra>",
            ))
            fig.add_trace(go.Bar(
                x=x_idx, y=s2_pct, name=s2_label, marker_color=s2_color, marker_line_width=0,
                width=0.35, offset=0.2, cliponaxis=False,
                text=[f"{v:.0f}%" if v is not None else "" for v in s2_pct],
                textposition="inside", textfont=dict(size=10, color="white", family="Arial"),
                hovertemplate=f"{s2_label}: %{{y:.1f}}%<extra></extra>",
            ))
            lay = _tlayout(title, 430)
            lay["showlegend"] = False   # row annotations below serve as the legend
            lay["yaxis"]["title"] = dict(text=yt, font=dict(size=10, color="#666"))
            lay["yaxis"]["ticksuffix"] = "%"
            lay["yaxis"]["range"] = [0, 105]
            lay["yaxis"]["domain"] = [0.42, 1.0]
            lay["xaxis"].update(dict(
                domain=[0.18, 1.0],
                tickmode="array",
            ...
            def _row(label, color, num, den, y_pos):
                fig.add_annotation(x=0.01, y=y_pos, xref="paper", yref="paper",
                    text=f"■ {label}", showarrow=False,
                    font=dict(size=10, color=color, family="Arial"), xanchor="left")
                for i, (n, d) in enumerate(zip(num, den)):
                    txt = f"{n:,}/{d:,}" if (n is not None and d is not None) else ""
                    fig.add_annotation(x=x_idx[i], y=y_pos, xref="x", yref="paper",
                        text=txt, showarrow=False,
                        font=dict(size=9, color="#374151", family="Arial"))
            _row(s1_label.replace(" (%)", ""), s1_color, s1_num, s1_den, 0.28)
            _row(s2_label.replace(" (%)", ""), s2_color, s2_num, s2_den, 0.13)
            return fig

        _hs_ext_live, _hs_ext_n, _hs_ext_d = _pg_live_pct_and_counts(
            "HS External Audit Sites", "Total no. of sites")
        _hs_int_live, _hs_int_n, _hs_int_d = _pg_live_pct_and_counts(
            "HS Internal Audit Sites", "Total no. of sites")
        _wb_live, _wb_n, _wb_d = _pg_live_pct_and_counts("Female Board", "Board Total")
        _wt_live, _wt_n, _wt_d = _pg_live_pct_and_counts("Female Employees", "Total Employees")
        ...
        with c1:
            if _has_hs:
                st.plotly_chart(_pg_bar_chart(
                    yrs, _hs_ext_live, _hs_ext_n, _hs_ext_d, "External H&S audit (%)", TC["bar_blue"],
                    _hs_int_live, _hs_int_n, _hs_int_d, "Internal H&S audit (%)", TC["bar_orange"],
                    "H&S system audit coverage (%)", "Sites audited (%)",
        ...
        with c2:
            if _has_div:
                st.plotly_chart(_pg_bar_chart(
                    yrs, _wt_live, _wt_n, _wt_d, "Women in workforce (%)", TC["bar_blue"],
                    _wb_live, _wb_n, _wb_d, "Women on Board (%)", TC["bar_orange"],
