...
def page_verification():
    ...
    _TAB_PREFIX_MAP = {
        "waste": "Waste",
        "pg":    "People & Governance",
        "elec":  "Electricity by Country",
    }
    def _tab_label(field_key: str) -> str:
        if ":" in field_key:
            pfx, _, _ = field_key.partition(":")
            return _TAB_PREFIX_MAP.get(pfx, pfx.title())
        if field_key == "SUBMISSION":
            return "Data Submission (Submit Data)"
        return "Main KPI Template"

    if pending_comments:
        # Group multiple field-level comments from the SAME submission event
        # (same Company + Year + Tab) into a single review card. Editing 5
        # fields in one Submit Data pass previously produced 5 separate
        # queue entries — each field is still tracked individually
        # underneath (so its own historical cell still turns red/bold on
        # its own), but reviewed here as one card with one batch action.
        from collections import defaultdict
        _groups = defaultdict(list)
        for cmt in pending_comments:
            _tab = _tab_label(cmt.get("FieldKey", ""))
            _groups[(cmt["Company"], cmt["Year"], _tab)].append(cmt)
        ...
        st.markdown(f"""<div style="border-left:4px solid {AMBER};padding:6px 12px;
          ...
          <b>🔔 {len(_groups)} pending change request(s) from clients</b>
        ...
        for idx_c, ((_co, _yr, _tab), _cmts) in enumerate(_groups.items()):
            _latest_ts = max(c["SubmittedAt"] for c in _cmts)
            _field_count = f" · {len(_cmts)} field(s)" if len(_cmts) > 1 else ""
            with st.expander(
                f"🏢 {_co} · {_yr} · {_tab}{_field_count} · submitted {_latest_ts}",
                expanded=True
            ):
                cc1, cc2 = st.columns([3, 1])
                with cc1:
                    st.markdown(f"**Tab:** {_tab}")
                    for _c in _cmts:
                        st.markdown(f"**Field:** {_c['FieldLabel']}  \n**Reason given:** {_c['Reason']}")
                        if _c is not _cmts[-1]:
                            st.markdown("—")
                with cc2:
                    ...
                    if _ba.button("✅ Accept", key=f"accept_{idx_c}",
                                  ...
                                  help="All field(s) in this group disappear from both templates"):
                        for _c in _cmts:
                            _update_comment_status(_c['Company'], int(_c['Year']),
                                                   _c['FieldKey'], "Accepted", approved_by=_analyst)
                        for _ck in list(st.session_state.keys()):
                            if _ck.startswith(f"tbl_editor_{_co}_{_yr}"):
                                del st.session_state[_ck]
                        st.success("✅ Accepted — comment(s) removed from both templates.")
                        st.rerun()
                    if _bs.button("⏳ Seen", key=f"seen_{idx_c}",
                        ...
                        for _c in _cmts:
                            _update_comment_status(_c['Company'], int(_c['Year']),
                                                   _c['FieldKey'], "Seen", approved_by=_analyst)
                        for _ck in list(st.session_state.keys()):
                            if _ck.startswith(f"tbl_editor_{_co}_{_yr}"):
                                del st.session_state[_ck]
                    ...
                    if _br.button("⚠ Reject", key=f"reject_{idx_c}",
                        ...
                        for _c in _cmts:
                            _update_comment_status(_c['Company'], int(_c['Year']),
                                                   _c['FieldKey'], "Rejected", approved_by=_analyst)
                        for _ck in list(st.session_state.keys()):
                            if _ck.startswith(f"tbl_editor_{_co}_{_yr}"):
                                del st.session_state[_ck]
