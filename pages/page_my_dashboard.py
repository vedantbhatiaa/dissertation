...
def page_my_dashboard():
    ...
    def _col_raw(col, divisor=1.0):
        """Per-year, per-company master-CSV read — same direct filter pattern
        page_home.py's _master_col() uses (one boolean mask per year, fresh
        off state.CONSOLIDATED_DF), rather than a single pre-sliced/indexed
        co_df.set_index('Year') built once up front. The pre-built version is
        exposed to a non-unique Year index if this company ever picked up a
        duplicate (Company, Year) row (e.g. a stub row from an earlier save
        path) — .loc[y, col] then returns a Series instead of a scalar for
        that year, silently breaking just that column. Filtering fresh per
        year with .iloc[0] sidesteps that entirely, matching the same
        approach already proven correct elsewhere in this codebase."""
        ...
        for y in yr_range:
            ...
            if row.empty:
                ...
                continue
            v = row[col].iloc[0]
            out.append(float(v) / divisor if pd.notna(v) else None)
        return out
    ...
    # "% certified sites" stored as 0-1 fraction → multiply by 100
    iso_cert         = [v * 100 if v is not None else None
                        for v in _col_raw("% certified sites")]
    # Waste columns — read whichever name exists in the live master CSV.
    # The master may use "Total Waste" (alias) or "Total amount of Waste" (canonical).
    def _waste_col(primary, fallback):
        vals = _col_raw(primary)
        if all(v is None for v in vals):
            vals = _col_raw(fallback)
        return vals

    total_waste  = _waste_col("Total amount of Waste",           "Total Waste")
    waste_recov  = _waste_col("Amount of waste sent to recovery", "Waste Recovered")
    # Recovery rate: try pre-computed column first, else compute from totals
    _raw_rate = _waste_col("Waste recovery %", "Waste_Recovery_Rate_%")
    waste_recov_rate = [
        v * 100 if (v is not None and v <= 1.01) else v   # normalise 0-1 → 0-100
        if v is not None else
        (r / t * 100 if (r is not None and t is not None and t > 0) else None)
        for v, r, t in zip(_raw_rate,
                           _waste_col("Amount of waste sent to recovery","Waste Recovered"),
                           _waste_col("Total amount of Waste","Total Waste"))
    ]
    production       = _col_raw("Production")
    # Renewable share: derive from raw electricity columns (not pre-stored)
    _renew_raw   = _col_raw("Renewable Electricity Purchased")
    _nonrenew_raw= _col_raw("Non-Renewable Electricity Purchased")
    renew_pct    = [
        round(r / (r + n) * 100, 1)
        if r is not None and n is not None and (r + n) > 0 else None
        for r, n in zip(_renew_raw, _nonrenew_raw)
    ]

    hs_ext_sites = _col_raw("HS External Audit Sites")
    hs_int_sites = _col_raw("HS Internal Audit Sites")
    # Prefer deriving from raw site counts — the precomputed "%" columns have
    # been confirmed (via the sector-aggregate view) to sometimes contain
    # corrupted values from bad data entry, producing nonsensical 800%+
    # readings. Raw counts are far less likely to be wrong. Clamp either way
    # as a hard safety net.
    def _clamp_pct(vals):
        return [max(0.0, min(100.0, v)) if v is not None else None for v in vals]

    hs_ext_pct = [a if a is not None else b for a, b in
                 zip(_pct(hs_ext_sites, total_sites),
                     [v * 100 if v is not None else None for v in _col_raw("HS External Audit %")])]
    hs_int_pct = [a if a is not None else b for a, b in
                 zip(_pct(hs_int_sites, total_sites),
                     [v * 100 if v is not None else None for v in _col_raw("HS Internal Audit %")])]
    hs_ext_pct, hs_int_pct = _clamp_pct(hs_ext_pct), _clamp_pct(hs_int_pct)
    ...
    fem_emp_pct  = [a if a is not None else b for a, b in
                   zip(_pct(emp_female, emp_total),
                       [v * 100 if v is not None else None for v in _col_raw("Female Employees %")])]
    fem_bod_pct  = [a if a is not None else b for a, b in
                   zip(_pct(bod_female, bod_total),
                       [v * 100 if v is not None else None for v in _col_raw("Female Board %")])]
    fem_emp_pct, fem_bod_pct = _clamp_pct(fem_emp_pct), _clamp_pct(fem_bod_pct)
    ...
    wrr = []
    for _a, _r, _t in zip(waste_recov_rate, waste_recov, total_waste):
        if _r is not None and _t and _t > 0:
            wrr.append(_r / _t * 100)
        else:
            wrr.append(_a)

    fuel_map = {
        "Natural Gas": "Energy - Natural Gas", "Coal": "Energy - Total Coal", "LPG": "Energy - LPG",
        "Fuel Oil": "Energy - Fuel Oil", "Diesel": "Energy - Diesel", "Petrol": "Energy - Petrol",
    ...
    with tab_energy:
        ...
        with col1:
            ...
            fig1.update_layout(margin=dict(l=160, r=60, t=110, b=240))
        ...
        with col_e_new1:
            ...
            fig_elec.update_layout(**_base("Electricity from renewable sources (%)",
                height=H, bottom_margin=220, top_margin=85))
            fig_elec.update_layout(barmode="stack")
            fig_elec.update_yaxes(range=[0, 100], ticksuffix="%", title=dict(text="Electricity (%)", font=dict(size=10)))
            _add_values_below_bars(fig_elec, xs, [
                ("<span style=\\'color:#3DBDB5\\'>■</span> Renewable (GJ)", _renew_raw, "{:,.0f}"),
                ("<span style=\\'color:#D4A574\\'>■</span> Non-renewable (GJ)", _nonrenew_raw, "{:,.0f}")], height=H)
            fig_elec.update_layout(margin=dict(l=160, r=60, t=85, b=240))
        ...
        with col_e_new2:
            ...
            _add_values_below_bars(fig_iso, xs, [
                ("<span style=\\'color:#3DBDB5\\'>■</span> Certified sites", iso_sites, "{:,.0f}"),
                ("<span style=\\'color:#9CA3AF\\'>■</span> Total sites", total_sites, "{:,.0f}")], height=H)
            fig_iso.update_layout(margin=dict(l=160, r=60, t=110, b=240))
    ...
    with tab_co2:
        ...
        with col1:
            ...
            fig3.update_layout(margin=dict(l=160, r=60, t=110, b=240))
        ...
        with col2:
            ...
            scope1_co2 = _col_raw("Total CO2 - Scope 1")  # real Scope 1 from master
    ...
    with tab_water:
        ...
        with col1:
            ...
            fig4.update_layout(margin=dict(l=160, r=60, t=110, b=240))
    ...
    with tab_waste:
        _has_waste_abs = any(v is not None and v != 0 for v in total_waste)
        _has_waste_rd  = any(v is not None and v != 0 for v in waste_recov) or \
                          any(v is not None and v != 0 for v in waste_elim)
        col1, col2 = st.columns(2, gap="medium")
        with col1:
            if not _has_waste_abs:
                diag_years = ", ".join(str(y) for y in yr_range)
                st.markdown(
                    f'<div class="dash-info-card">📋 <b>Waste Generated & Intensity</b><br>'
                    f'No waste data found for {company} in {diag_years}. '
                    'Submit via Submit Data → Section 6, or switch the time range above to "All years".</div>',
                    unsafe_allow_html=True)
            else:
                H = 520
                fig6 = go.Figure()
                ...
                fig6.update_layout(margin=dict(l=160, r=60, t=110, b=240))
                apply_chart_animation(fig6)
                _card_open()
                st.plotly_chart(fig6, use_container_width=True, key=_chart_key(company, "f6"))
                _card_close()
        with col2:
            if not _has_waste_rd:
                diag_years = ", ".join(str(y) for y in yr_range)
                st.markdown(
                    f'<div class="dash-info-card">📋 <b>Waste Recovery vs Elimination</b><br>'
                    f'No waste recovery data found for {company} in {diag_years}. '
                    'Submit via Submit Data → Section 6, or switch the time range above to "All years".</div>',
                    unsafe_allow_html=True)
            else:
                H = 520
        ...
        with col1:
            ...
            if not has_hs:
                ...
                fig8.update_layout(**_simple_bar_layout(
                    ...
                    "% sites with audited H&S system", height=H, bottom_margin=220))
                fig8.update_yaxes(range=[0, 105], ticksuffix="%")
                _add_values_below_bars(fig8, xs, [
                    ("<span style=\\'color:#B8CDD9\\'>■</span> Ext. audited (sites)", hs_ext_sites, "{:,.0f}"),
                    ("<span style=\\'color:#F5A623\\'>■</span> Int. audited (sites)", hs_int_sites, "{:,.0f}")], height=H)
                fig8.update_layout(margin=dict(l=160, r=60, t=110, b=240))
        ...
        with col2:
            ...
            if not has_ppl:
                ...
                fig9.update_layout(**_simple_bar_layout(
                    ...
                    "Female representation (%)", height=H, bottom_margin=220))
                fig9.update_yaxes(range=[0, _y9], ticksuffix="%")
                _add_values_below_bars(fig9, xs, [
                    ("<span style=\\'color:#B8CDD9\\'>■</span> Women employees", emp_female, "{:,.0f}"),
                    ("<span style=\\'color:#F5A623\\'>■</span> Women on Board", bod_female, "{:,.0f}")], height=H)
                fig9.update_layout(margin=dict(l=160, r=60, t=110, b=240))
