...
def page_home():
    ...
    prev_yr_note = f"<b>{prev_yr}</b> vs " if prev_out is not None else ""
    if has_data_for_sel_yr:
        footnote = (
            f"KPI values shown for {prev_yr_note}<b>{display_yr}</b>. "
        ...
        footnote = (
            ...
            f"reported year, {prev_yr_note}<b>{display_yr}</b>, instead. "
    ...
    for y in years:
        ...
        # Always compute % from raw site/headcount counts.
        # The stored % columns in master are inconsistent (some years 0-1,
        # some years 0-100 depending on which save path wrote them).
        # Computing directly from counts is always correct and matches
        # what render_people_tab.py and My Records show.
        _hs_tot = _master_col("HS Total Sites") or total_sites_y or 1
        hs_ext_pct  = _ratio_pct(_master_col("HS External Audit Sites"), _hs_tot)
        hs_int_pct  = _ratio_pct(_master_col("HS Internal Audit Sites"), _hs_tot)
