...
def get_hist_outputs():
    ...
    # Cache key: invalidates when company changes OR when the company's data
    # actually changes (not just row count — that doesn't change on row update).
    # We use the sum of Total energy for this company as a fast value-change
    # proxy: it changes on any real KPI save without hashing 734 columns.
    _df = state.CONSOLIDATED_DF
    if not _df.empty and "Company" in _df.columns and "Total energy" in _df.columns:
        _co_slice = _df[_df["Company"] == company]["Total energy"]
        _data_sig = round(float(_co_slice.sum()), 0) if not _co_slice.empty else 0
    else:
        _data_sig = 0
    _key = f"_hist_cache|{company}|{state.CURR_YEAR}|{_data_sig}"
    if st.session_state.get(_key) is not None:
        return st.session_state[_key]

    if company and not _df.empty:
        co_years  = sorted(dl.get_years(_df, company) or [])
        # Slice company rows ONCE — avoids re-filtering the full 734-col DF
        # on every iteration. get_company_hist returns a year-keyed dict so
        # get_step_data is just a dict lookup + column iteration, not a filter.
        comp_hist = dl.get_company_hist(_df, company)
    ...
    st.session_state[_key] = outs
    # Also store the key name so data_utils can bust it without iterating all keys
    st.session_state["_hist_cache_last_key"] = _key
    return outs
