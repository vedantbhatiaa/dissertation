...
# ── Permanent "edited" log ──────────────────────────────────────────────────
# Separate from field_comments.csv, which only tracks the CURRENT comment
# state and has its record deleted once a comment is Accepted. This log is
# append-only and never cleared — it exists purely so a corrected historical
# cell can stay permanently red/bold, independent of whatever later happens
# to the comment itself (Seen / Rejected / Accepted).
_EDITED_LOG_PATH = Path("data_storage/edited_fields_log.csv")
_EDITED_LOG_COLS = ["Company", "Year", "FieldKey", "FirstEditedAt"]


def _log_edited_field(company: str, year, field_key: str) -> None:
    """Permanently record that this Company+Year+FieldKey was corrected via
    the change-comment workflow. Idempotent — writing the same field twice
    doesn't duplicate the row. Never deleted, including on comment Accept."""
    from datetime import datetime
    try:
        _EDITED_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
        rows = []
        if _EDITED_LOG_PATH.exists():
            with open(_EDITED_LOG_PATH, newline="", encoding="utf-8") as f:
                rows = list(csv.DictReader(f))
        already = any(r["Company"] == company and str(r["Year"]) == str(year)
                      and r["FieldKey"] == field_key for r in rows)
        if not already:
            rows.append({
                "Company": company, "Year": str(year), "FieldKey": field_key,
                "FirstEditedAt": datetime.now().strftime("%Y-%m-%d %H:%M"),
            })
            with open(_EDITED_LOG_PATH, "w", newline="", encoding="utf-8") as f:
                w = csv.DictWriter(f, fieldnames=_EDITED_LOG_COLS)
                w.writeheader(); w.writerows(rows)
    except Exception as e:
        logger.warning("[comment_utils] _log_edited_field: %s", e)


def get_edited_field_years(company: str) -> set[tuple[str, str]]:
    """Return {(Year, FieldKey)} pairs permanently marked as edited for this
    company, across all years. Used by the KPI templates to keep a corrected
    cell red/bold forever, regardless of the comment's current status."""
    if not _EDITED_LOG_PATH.exists():
        return set()
    try:
        with open(_EDITED_LOG_PATH, newline="", encoding="utf-8") as f:
            rows = list(csv.DictReader(f))
        return {(r["Year"], r["FieldKey"]) for r in rows if r["Company"] == company}
    except Exception as e:
        logger.warning("[comment_utils] get_edited_field_years: %s", e)
        return set()
...
def save_change_comment(company: str, year, field_key: str, field_label: str,
    ...
    try: _log_edited_field(company, year, field_key)
    except Exception: pass
...
def save_comment_version(company: str, year, field_key: str, reason: str,
    ...
    """
    Save a lightweight version snapshot recording a single comment event
    ...
    try:
        ...
        logger.warning("[comment_utils] save_comment_version (local fallback): %s", e)